...
class blefuzzer:
    def read_input(self):
        ...
        try:
            self.chars_to_write = input(
                "How many characters(maximum) you would like to write to GATT handle?: ")
        except:
            print('Please enter valid number!')
            sys.exit(1)

        print('Hint: If BLE peripheral is not accepting connections with default <public> LE Address type, input <random>')
        self.le_address_type = input(
            "Set LE address type (public/ random): ")
        self.mac_address = sys.argv[1]

    # Fuzz all handles to identify valid ones; Read GATT Characteristics from valid handles
    def read_handle(self):
        try:
            ...
            ble_primary_service = 'gatttool --addr-type=%s --primary --device=%s' % (
                self.le_address_type, self.mac_address)
            ...
            for each_combination in itertools.product(char_handle_all_combinations, char_handle_all_combinations, char_handle_all_combinations, char_handle_all_combinations):
                ...
                characteristic_read = 'gatttool --addr-type=%s --device=%s --char-read --handle=%s' % (
                    self.le_address_type, self.mac_address, characteristic_handle)

                while True:  # Try until succesful connection is made to read a characteristics from BLE device
                    process = subprocess.Popen(shlex.split(
                        characteristic_read), stdout=subprocess.PIPE)
                    stdout = process.communicate()
                    # exit code of the process. '0'-if all good; '1'-if something is bad; 'None'- if the process is still executing
                    ...
                    if stdout[0] != '' and exit_code == 0:
                        ...
                        regex = re.search(
                            '^Characteristic value\/descriptor:[\s]+(.*)', characeteristic)
                        clean_charecteristic = regex.groups()[0]
                        # Pushing readable handles to dictionary
                        readable_handle[characteristic_handle] = clean_charecteristic
    ...
    def write_handle(self):
        ...
        for each_combination in itertools.product(char_handle_all_combinations, char_handle_all_combinations, char_handle_all_combinations, char_handle_all_combinations):
            ...
            for char_value_length in range(self.chars_to_write):
                value += str(0)
                cmd = 'gatttool --addr-type=%s --device=%s --char-write-req --handle=%s --value=%s' % (
                    self.le_address_type, self.mac_address, characteristic_handle, value)
                ...
                while True:
                    try:
                        ...
                        exit_code = process.poll()
                        print(characteristic_handle, ':')
                        print('Trying to write %s characters:' %
                              (char_value_length+1))
                        if stdout[0] != '' and exit_code == 0:
                            print(stdout[0])
                            print('Written %s chars' % (char_value_length+1))
                            handle_values_list = handle_values.append(
                                char_value_length+1)
                            break
                        ...
                        print(
                            'Seems I am having issues reaching BLE Device. Will try again...', e)
                        continue
...
class ble_fuzzer_quick:
    def read_input(self):
        ...
        print('Hint: If BLE peripheral is not accepting connections with default <public> LE Address type, input <random>')
        self.le_address_type = raw_input(
            "Set LE address type (public/ random): ")
        self.mac_address = sys.argv[1]
    ...
    def read_handle(self):
        try:
            ...
            # print Primary services offered by BLE device/peripheral
            print('BLE Primary Services: ')
            ble_primary_service = 'gatttool --addr-type=%s --primary -b %s' % (
                self.le_address_type, self.mac_address)
            process = subprocess.Popen(shlex.split(
                ble_primary_service), stdout=subprocess.PIPE)
            stdout = process.communicate()
            # exit code of the process. '0'-if all good; '1'-if something is bad; 'None'- if the process is still executing
            exit_code = process.poll()
            print(stdout[0])
            ...
            # Picks alternative values in a list starting from first
            group1 = primary_services[0:length:2]
            # Picks alternative values in a list starting from second
            group2 = primary_services[1:length:2]
            self.group1 = group1
            self.group2 = group2
            print('\n')
            print('Starting to read characteristics... \n')

            # Picks handle (same index) from group1 and group2
            for handle_from_group1, handle_from_group2 in zip(group1, group2):
                # Converts string handle to decimal handle
                handle_from_group1 = int(handle_from_group1, 16)
                handle_from_group2 = int(handle_from_group2, 16)
                # Iterates through all handles in the range. ex: 0001-000f
                for handle in range(handle_from_group1, handle_from_group2+1):
                    current_handle = '0x%04x' % handle
                    print('%s: ' % current_handle)
                    characteristic_read = 'gatttool --addr-type=%s --device=%s --char-read --handle=%s' % (
                        self.le_address_type, self.mac_address, current_handle)
                    while True:  # Try until succesful connection is made to read a characteristics from BLE device
                        process = subprocess.Popen(shlex.split(
                            characteristic_read), stdout=subprocess.PIPE)
                        stdout = process.communicate()
                        ...
                        if stdout[0] != '' and exit_code == 0:
                            ...
                            # Disable this line if you dont want the output to be displayed on screen
                            print(clean_charecteristic)
                            # Pushing readable handles to dictionary
                            readable_handle[current_handle] = clean_charecteristic
    ...
    def write_handle(self):
        ...
        handle_writable_chars = {}  # Holds writable character length for each handle
        # Picks handle (same index) from group1 and group2
        for handle_from_group1, handle_from_group2 in zip(self.group1, self.group2):
            # Converts string handle to decimal handle
            handle_from_group1 = int(handle_from_group1, 16)
            handle_from_group2 = int(handle_from_group2, 16)
            # Iterates through all handles in the range. ex: 0001-000f
            for handle in range(handle_from_group1, handle_from_group2+1):
                ...
                for char_value_length in range(self.chars_to_write):
                    value += str(0)
                    cmd = 'gatttool --addr-type=%s --device=%s --char-write-req --handle=%s --value=%s' % (
                        self.le_address_type, self.mac_address, current_handle, value)
...
if __name__ == '__main__':
    ...
    fuzz_type = input(
        "Do you wish to fuzz all the GATT handles(65536) or only handles specific to "
		"GATT primary services? Fuzzing all GATT handles takes ages. Type 'y' if yes: ")
    if fuzz_type == 'y' or fuzz_type == 'Y':
        fuzz = blefuzzer()
        read_inputs = fuzz.read_input()

        attribute_type = raw_input(
            "Do you wish to read values from all the GATT handles before writing "
			"values to it? If yes, type 'y': ")

        if ('y', 'Y') in attribute_type:
